...
export default function MessagesPage() {
  ...
  const [activeConversation, setActiveConversation] = useState<string | null>(searchParams.get("connectionId"))
  ...
  useEffect(() => {
    ...
    async function fetchMessages() {
      try {
        const res = await fetch(`/api/messages?connectionId=${activeConversation}&userId=${user.id}`)
        if (res.ok) {
          const data = await res.json()
  ...
  const activeConv = conversations.find((c) => c.connectionId === activeConversation)
  ...
  if (conversations.length === 0) {
    return (
      <div className="mx-auto max-w-6xl px-4 py-8">
        ...
        <p className="text-muted-foreground">No conversations yet. Accept a post to start messaging!</p>
      </div>
    )
  }
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-baseline justify-between gap-2">
                  <div className="truncate text-sm font-medium">{conv.otherUser?.name || "Unknown User"}</div>
                  {conv.lastMessage && (
                ...
                {conv.post && <div className="truncate text-xs text-muted-foreground">Re: {conv.post.title}</div>}
                {conv.lastMessage && (
                  <div className="truncate text-xs text-muted-foreground">{conv.lastMessage.text}</div>
                )}
              </div>
            </button>
          ))}
        ...
        {activeConv && (
          <div className="border-b p-4">
            <div className="flex items-center gap-3">
              <Avatar>
                <AvatarFallback>{activeConv.otherUser?.name.charAt(0).toUpperCase() || "?"}</AvatarFallback>
              </Avatar>
              <div>
                <div className="font-medium">{activeConv.otherUser?.name || "Unknown User"}</div>
                {activeConv.post && <div className="text-xs text-muted-foreground">Re: {activeConv.post.title}</div>}
              </div>
            </div>
          </div>
        )}
