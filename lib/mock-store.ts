...
let posts: Post[] = [
  {
    ...
    description:
      "Experienced tutor available for front-end lessons. Flexible hours.",
    category: "Technology",
  ...
  },
  {
    ...
    description:
      "Looking for a native Spanish speaker for conversation practice.",
    category: "Languages",
...
export function listPosts(filters?: {
  ...
  return posts.filter((p) => {
    ...
    const matchDept =
      !department || department === "all" || p.department === department;
...
export function createPost(input: {
  ...
  const post: Post = {
    _id: `p_${Math.random().toString(36).slice(2, 9)}`,
...
export function updatePost(
  id: string,
  patch: Partial<
    Pick<Post, "title" | "description" | "category" | "department" | "type">
  >
): Post | undefined {
